...
- Always opt for an online-first approach (prefer network-backed data and flows; if offline, fail gracefully with clear user-facing errors unless offline support is explicitly requested).
...
4. Run targeted validation checks:
   - `dart format .`
   - `flutter analyze`
5. Skip running `flutter test` unless explicitly requested by the user.
6. If a command is unavailable because the repository is not fully scaffolded yet, report that clearly and continue with safe, minimal edits.
